...
struct pthreadpool_executor {
  ...
  int (*num_threads)(struct pthreadpool_executor*);

  /// Schedule `task` to be called, with `context` as its argument.
  void (*schedule)(struct pthreadpool_executor*, void* context,
 ...
 * max_num_threads and @a executor->num_threads(scheduler) calls to @a
 * executor->schedule(executor,...) will be executed, potentially lasting for
 ...
 *
 * Note that the number of threads can only be adjusted if the @a threadpool was
 * created using @a pthreadpool_create_v2 with an external @a
 * pthreadbool_scheduler.
 *
 * The maximum number of threads that can be set is given by the @a
 * max_num_threads parameter to @a pthreadpool_create_v2.
 *
 * @param  threadpool   the thread pool to query.
...
class PthreadpoolExecutor : public pthreadpool_executor {
 ...
 private:
  static int num_threads_impl(struct pthreadpool_executor* executor) {
    return static_cast<PthreadpoolExecutor*>(executor)->NumThreads();
  }

  static void schedule_impl(struct pthreadpool_executor* executor,
                            void* context, TaskFunction task) {
    static_cast<PthreadpoolExecutor*>(executor)->Schedule(context, task);
  }
};
