...
class SimpleThreadPool : public PthreadpoolScheduler {
 public:
  using Task = std::pair<TaskFunction, void*>;

  SimpleThreadPool(uint32_t num_threads_) {
    for (int k = 0; k < num_threads_; k++) {
      threads_.emplace_back(SimpleThreadPool::ThreadMain, this);
    }
  }
  ~SimpleThreadPool() {
    {
 ...
 private:
  static void ThreadMain(SimpleThreadPool* pool) {
    Task task;
    while (true) {
      {
        std::unique_lock lock(pool->task_mutex_);
        while (pool->tasks_.empty()) {
          if (pool->done_) {
            return;
          }
          pool->task_cond_var_.wait(lock);
        }
        task = std::move(pool->tasks_.front());
        pool->tasks_.pop_front();
        if (pool->tasks_.empty()) {
          pool->empty_cond_var_.notify_all();
        }
      }
...
TEST(CreateAndDestroy, SingleThreadPool) {
  ...
  pthreadpool* threadpool = pthreadpool_create_v2(&pool, 1);
...
TEST(CreateAndDestroy, MultiThreadPool) {
  SimpleThreadPool pool(8);
  pthreadpool* threadpool = pthreadpool_create_v2(&pool, 0);
  ASSERT_TRUE(threadpool);
  pthreadpool_destroy(threadpool);
}
...
TEST(Parallelize1D, MultiThreadPoolCompletes) {
  SimpleThreadPool pool(8);
  auto_pthreadpool_t threadpool(pthreadpool_create_v2(&pool, 0),
                                pthreadpool_destroy);
...
static void WorkImbalanceHelper(std::atomic_int* num_processed_items,
...
static void WorkImbalance1D(std::atomic_int* num_processed_items, size_t i) {
  WorkImbalanceHelper(num_processed_items, 1, kParallelize1DRange, i == 0);
}
...
static void WorkImbalance1DTile1D(std::atomic_int* num_processed_items,
                                  size_t start_i, size_t tile_i) {
  WorkImbalanceHelper(num_processed_items, tile_i, kParallelize1DTile1DRange,
                      start_i == 0);
}
...
TEST(Parallelize1DTile1DDynamic,
  ...
  SimpleThreadPool pool(8);
  auto_pthreadpool_t threadpool(pthreadpool_create_v2(&pool, 1),
                                pthreadpool_destroy);
  ASSERT_TRUE(threadpool.get());
...
static void WorkImbalance2D(std::atomic_int* num_processed_items, size_t i,
                            size_t j) {
  WorkImbalanceHelper(num_processed_items, 1,
                      kParallelize2DRangeI * kParallelize2DRangeJ,
                      i == 0 && j == 0);
}
...
static void WorkImbalance2DTile1D(std::atomic_int* num_processed_items,
                                  size_t i, size_t start_j, size_t tile_j) {
  WorkImbalanceHelper(num_processed_items, tile_j,
                      kParallelize2DTile1DRangeI * kParallelize2DTile1DRangeJ,
                      i == 0 && start_j == 0);
}
...
static void WorkImbalance2DTile2D(std::atomic_int* num_processed_items,
                                  size_t start_i, size_t start_j, size_t tile_i,
                                  size_t tile_j) {
  WorkImbalanceHelper(num_processed_items, tile_i * tile_j,
                      kParallelize2DTile2DRangeI * kParallelize2DTile2DRangeJ,
                      start_i == 0 && start_j == 0);
}
...
static void WorkImbalance3DTile1D(std::atomic_int* num_processed_items,
                                  size_t i, size_t j, size_t start_k,
                                  size_t tile_k) {
  WorkImbalanceHelper(num_processed_items, tile_k,
                      kParallelize3DTile1DRangeI * kParallelize3DTile1DRangeJ *
                          kParallelize3DTile1DRangeK,
                      i == 0 && j == 0 && start_k == 0);
}
...
static void WorkImbalance3DTile2D(std::atomic_int* num_processed_items,
                                  size_t i, size_t start_j, size_t start_k,
                                  size_t tile_j, size_t tile_k) {
  WorkImbalanceHelper(num_processed_items, tile_j * tile_k,
                      kParallelize3DTile2DRangeI * kParallelize3DTile2DRangeJ *
                          kParallelize3DTile2DRangeK,
...
static void WorkImbalance4DTile1D(std::atomic_int* num_processed_items,
                                  size_t i, size_t j, size_t k, size_t start_l,
                                  size_t tile_l) {
  WorkImbalanceHelper(num_processed_items, tile_l,
                      kParallelize4DTile1DRangeI * kParallelize4DTile1DRangeJ *
                          kParallelize4DTile1DRangeK *
                          kParallelize4DTile1DRangeL,
...
static void WorkImbalance4DTile2D(std::atomic_int* num_processed_items,
                                  ...
                                  size_t start_l, size_t tile_k,
                                  size_t tile_l) {
  WorkImbalanceHelper(num_processed_items, tile_k * tile_l,
                      kParallelize4DTile2DRangeI * kParallelize4DTile2DRangeJ *
                          kParallelize4DTile2DRangeK *
                          kParallelize4DTile2DRangeL,
...
static void WorkImbalance5DTile1D(std::atomic_int* num_processed_items,
                                  size_t i, size_t j, size_t k, size_t l,
                                  size_t start_m, size_t tile_m) {
  WorkImbalanceHelper(num_processed_items, tile_m,
                      kParallelize5DTile1DRangeI * kParallelize5DTile1DRangeJ *
                          kParallelize5DTile1DRangeK *
                          kParallelize5DTile1DRangeL *
                          kParallelize5DTile1DRangeM,
...
static void WorkImbalance5DTile2D(std::atomic_int* num_processed_items,
                                  ...
                                  size_t tile_m) {
  WorkImbalanceHelper(
      num_processed_items, tile_l * tile_m,
      kParallelize5DTile2DRangeI * kParallelize5DTile2DRangeJ *
          kParallelize5DTile2DRangeK * kParallelize5DTile2DRangeL *
          kParallelize5DTile2DRangeM,
...
static void WorkImbalance6D(std::atomic_int* num_processed_items, size_t i,
  ...
  WorkImbalanceHelper(num_processed_items, 1,
                      kParallelize6DRangeI * kParallelize6DRangeJ *
                          ...
                          kParallelize6DRangeM * kParallelize6DRangeN,
...
static void WorkImbalance6DTile2D(std::atomic_int* num_processed_items,
                                  ...
                                  size_t tile_n) {
  WorkImbalanceHelper(
      num_processed_items, tile_m * tile_n,
      kParallelize6DTile2DRangeI * kParallelize6DTile2DRangeJ *
          kParallelize6DTile2DRangeK * kParallelize6DTile2DRangeL *
          kParallelize6DTile2DRangeM * kParallelize6DTile2DRangeN,
